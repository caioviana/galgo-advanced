...
namespace galgo
{
	...
	private:
		...
		std::vector<T> result;
		std::string chr;
		const GeneticAlgorithm<T> *ptr = nullptr;
	...
	template<class T> Chromosome<T>::Chromosome(const GeneticAlgorithm<T> &ga)
	{
	...
	template<class T> Chromosome<T>::Chromosome(const Chromosome<T> & rhs)
	{
		param    =rhs.param; 
		result   =rhs.result;
		chr      =rhs.chr;
